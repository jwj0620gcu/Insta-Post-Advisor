...
인스타그램 크리에이터라면 누구나 겪는 문제다. 열심히 만든 게시물이 왜 안 되는지, 어디를 고쳐야 하는지 알 방법이 없다. Insta-Advisor는 게시물을 올리기만 하면 5개 전문가 에이전트가 진단하고, 바로 쓸 수 있는 개선안을 제시한다.
...
- **오디오 STT** — 릴스 음성에서 텍스트 추출 (설계 구현)

### 2단계 — 이중 진단 모델 협업

**첫 번째 모델 — 정량 채점 (Model A)**

인스타그램 공개 API로 수집한 실제 게시물 96건(8개 카테고리 × 12건) 기반.  
LLM 없이 회귀 가중치만으로 5가지 지표를 즉시 산출한다.
...
- **모의 댓글 시뮬레이션** — 공감형·광고 지적형·질문형 등 실제 유저 반응 미리 가늠
