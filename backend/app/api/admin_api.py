...
async function doLogin(){
  ...
  if(!r.ok){showLogin('비밀번호 오류');return;}token=pw;showDash(await r.json());}catch(e){showLogin('연결 실패');}
}
...
function showDash(d){
  ...
  <div class="g">
    ...
    <div class="c"><div class="l">누적 진단</div><div class="v g2">${d.total_requests||0}</div></div>
    <div class="c"><div class="l">누적 UV</div><div class="v o">${d.unique_ips||0}</div></div>
    <div class="c"><div class="l">누적 Token</div><div class="v sm">${(d.total_tokens||0).toLocaleString()}</div></div>
    <div class="c"><div class="l">평균 지연</div><div class="v sm">${d.avg_duration_sec||0}s</div></div>
    <div class="c"><div class="l">학습 샘플</div><div class="v sm">${d.total_notes||0}</div></div>
    <div class="c"><div class="l">메모리</div><div class="v sm">${d.system?.memory_used_mb||'?'}/${d.system?.memory_total_mb||'?'}</div></div>
  </div>
  <div class="s">24시간 요청 분포</div>
  <div style="display:flex;gap:3px;align-items:end;height:60px;margin-bottom:16px">
    ...
    return `<div title="${h}:00 → ${v}건" style="flex:1;background:${v?'#ff2442':'#f0f0f0'};height:${Math.max(v/maxH*100,2)}%;border-radius:2px;cursor:pointer"></div>`;}).join('')}
  </div>
  <div class="s">카테고리 분포</div>
  ...
  <table><tr><th>IP</th><th>횟수</th><th>비율</th></tr>
  ...
  <div class="s">최근 진단</div>
  <table><tr><th>시간</th><th>IP</th><th>제목</th><th>카테고리</th><th>Token</th><th>지연</th><th>상태</th></tr>
  ...
  ${usage.length===0?'<tr><td colspan=7 style="color:#999;text-align:center">기록 없음</td></tr>':''}</table>
  <button class="refresh" onclick="doRefresh()">새로고침</button>
  <span id="autoLabel" style="font-size:11px;color:#999;margin-left:12px">30초 자동 새로고침</span></div>`;
}
...
async def admin_stats(password: str = Query(...)):
    if not _verify_password(password):
        raise HTTPException(403, "비밀번호 오류")
    return _get_stats()
